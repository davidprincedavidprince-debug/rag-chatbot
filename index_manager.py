...
def incremental_update(
    ...
    # ── Try HuggingFace restore before full rebuild ──────────────
    if not index_exists and not force_rebuild:
        print("No local index — checking HuggingFace …")
        try:
            from hf_store import download_index, hf_index_exists
            if hf_index_exists():
                print("Found index on HuggingFace — downloading …")
                if download_index():
                    index_exists = os.path.exists(
                        os.path.join(INDEX_PATH, "index.faiss")
                    )
                    if index_exists:
                        print("✅  Index restored from HuggingFace — skipping rebuild")
                        # Load and return immediately — no rebuild needed
                        vs = FAISS.load_local(
                            INDEX_PATH,
                            embeddings,
            ...
            print(f"HuggingFace restore skipped: {e}")
